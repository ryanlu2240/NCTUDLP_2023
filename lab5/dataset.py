...
class bair_robot_pushing_dataset(dataset):
	def __init__(self, args, mode="train", transform=default_transform):
		...
		## Get all data paths
		self.data_dirs = []
		self.mode_data_root = f"{self.args.data_root}/{self.mode}"
		for data_dir in os.listdir(self.mode_data_root):
			if data_dir[0] == ".":
				continue
			idx_dirs = os.listdir(f"{self.mode_data_root}/{data_dir}")
			for idx_dir in idx_dirs:
				if idx_dir[0] == ".":
					continue
				self.data_dirs.append(f"{self.mode_data_root}/{data_dir}/{idx_dir}")
	...
	def get_seq(self, index):
		...
		for frame_file in frame_files:
			img_arr = Image.open(f"{self.data_dirs[index]}/{frame_file}")
	...
	def get_csv(self, index):
		...
		conds = []
		files = [
			f"{self.data_dirs[index]}/actions.csv", 
			f"{self.data_dirs[index]}/endeffector_positions.csv"
		]
