...
## Self-defined
from utils import mse_metric, kl_criterion, plot_pred, plot_rec, finn_eval_seq, pred
...
class Trainer:
	...
	def train(
		...
		for epoch in range(start_epoch, start_epoch + niter):
			...
			progress.update(1)
			with open(f"{self.args.log_dir}/train_record.txt", "a") as train_record:
				train_record.write(
	...
	def test(self, test_data, test_loader, test_iterator, test_set="test"):
		...
		ave_psnr = np.mean(np.concatenate(psnr_list))
		print(f"[Epoch best] {test_set} psnr = {ave_psnr:.5f}")
