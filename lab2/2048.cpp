...
public:
	...
	virtual float estimate(const board& b) const {
		// std::cout<<"pattern iso"<<std::endl;
		// for(int i=0;i<8;i++){
		// 	for(int j=0;j<isomorphic[i].size();j++){
		// 		std::cout<<isomorphic[i][j]<<' ';
		// 	}
		// 	std::cout<<std::endl;
		// }
		// TODO
...
public:
	...
	float get_expect(const board& b) const{
		float expect = 0;
		bool is_moved = false;
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)){
				is_moved = true;
				float value = estimate(move->after_state());
				expect = move->reward() + value;
	...
	state select_best_move(const board& b, std::string mode) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				float value = estimate(s_after);
				// float value = 0;
				if(mode == "demo"){
					float e1 = 0;
					float e2 = 0;
					for(int i=0;i<empty_idx.size();i++){
						board s_prime2_2 = s_after;
						board s_prime2_4 = s_after;
						s_prime2_2.popup_specific(empty_idx[i], 1); // 2^1 = 2
						s_prime2_4.popup_specific(empty_idx[i], 2); // 2^2 = 4

						e1 += get_expect(s_prime2_2);
						e2 += get_expect(s_prime2_4);

					}
					value =  e1 * 9 / (10. * empty_idx.size()) + e2 * 1 / (10. * empty_idx.size());
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		for (path.pop_back(); path.size(); path.pop_back()) {
			...
			float error = move.reward() + value_s_prime2 - estimate(move.after_state());
			value_s_prime2 = update(move.after_state(), alpha * error);
		}
	}
...
int main(int argc, const char* argv[]) {
	...
	if (mode == "demo") {
		...
		tdl.load("best_d0.bin");

		std::vector<state> path;
		path.reserve(20000);
		info << "\nStart playing " << unit << " episodes\n";
		...
		for (size_t n = 1; n <= unit; n++) {
			...
			std::vector<float> data = tdl.make_statistic(n, b, score, unit);
		}
		return 0;
	}
	if(mode == "train"){
		info << "TDL2048-Train" << std::endl;
		//output data
		std::ofstream output_data;
		std::stringstream ss;
		output_data.open ("training_data.csv");
		std::string str = "";
		...
		for (size_t n = 1; n <= total; n++) {
			...
			data = tdl.make_statistic(n, b, score);
			if(n % unit == 0){
				ss << n << "," << data[0] << ",\n";
				output_data << ss.str();
				ss.str("");
